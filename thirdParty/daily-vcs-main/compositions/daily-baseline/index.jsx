...
export default function DailyBaselineVCS() {
      ...
      videoLabel: {
        ...
        fontSize_px: params["videoSettings.labels.fontSize_pct"]
          ? (params["videoSettings.labels.fontSize_pct"] / 100) *
            DEFAULT_LABEL_FONT_SIZE_PX
          : DEFAULT_LABEL_FONT_SIZE_PX,
        strokeColor: params["videoSettings.labels.strokeColor"] ||
          "rgba(0, 0, 0, 0.9)",
        strokeWidth_px: 4,
      },
    };
  ...
  const omitPausedVideo = params["videoSettings.omitPausedVideo"] != null
      ...
      case "grid":
        video = (
          <VideoGrid
            ...
            preserveItemAspectRatio={params[
      ...
      case "pip": {
        ...
        video = (
          <VideoPip
            ...
            disableRoundedCornersOnMain={params[
  ...
  if (params.showTextOverlay) {
    ...
    overlayProps.useStroke = overlayProps.strokeColor &&
...

  {
    ...
    const arr = params["image.zPosition"] === "foreground"
      ? graphics
      : bgGraphics;
  ...
  // lower third
  {
    ...
    const strokeWidth_px = params["lowerThird.stroke_gu"] > 0
      ? params["lowerThird.stroke_gu"] * pxPerGu
      : 0;
    const textStrokeWidth_px = params["lowerThird.text.stroke_gu"] > 0
    ...
    graphics.push(
      <LowerThird
        ...
        bgStyle={{
          ...
          strokeWidth_px,
          cornerRadius_px: parseFloat(params["lowerThird.cornerRadius_gu"]) *
            pxPerGu,
        }}
        textStyle={{
...

  {
    ...
    if (isFinite(params["titleSlate.subtitle.fontSize_pct"])) {
      subtitleStyle.fontSize_gu *= params["titleSlate.subtitle.fontSize_pct"] /
        100;
    }
  ...
  if (isFinite(params["videoSettings.margin.left_vw"])) {
    videoMargins_gu.l = parseFloat(params["videoSettings.margin.left_vw"]) *
      guPerVw;
  }
  if (isFinite(params["videoSettings.margin.right_vw"])) {
    videoMargins_gu.r = parseFloat(params["videoSettings.margin.right_vw"]) *
      guPerVw;
  }
  if (isFinite(params["videoSettings.margin.top_vh"])) {
    videoMargins_gu.t = parseFloat(params["videoSettings.margin.top_vh"]) *
      guPerVh;
  }
  if (isFinite(params["videoSettings.margin.bottom_vh"])) {
    videoMargins_gu.b = parseFloat(params["videoSettings.margin.bottom_vh"]) *
      guPerVh;
  }
  if (
