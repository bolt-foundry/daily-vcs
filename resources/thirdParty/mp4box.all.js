...
Log.getDurationString = function (duration, _timescale) {
  ...
  return (neg ? "-" : "") + hours + ":" + pad(minutes, 2) + ":" +
    pad(duration_sec, 2) + "." + pad(msec, 3);
};
...
Log.printRanges = function (ranges) {
  ...
  if (length > 0) {
    ...
    for (var i = 0; i < length; i++) {
      if (i > 0) str += ",";
      str += "[" + Log.getDurationString(ranges.start(i)) + "," +
    ...
    if (ab.fileStart <= b.fileStart) {
      ...
      if (ab.fileStart === b.fileStart) {
        ...
        if (ab.byteLength > b.byteLength) {
          ...
          Log.warn(
            "MultiBufferStream",
            "Buffer (fileStart: " + ab.fileStart + " - Length: " +
              ab.byteLength + ") already appended, ignoring",
          );
        }
      } else {
        ...
        Log.debug(
          "MultiBufferStream",
          "Appending new buffer (fileStart: " + ab.fileStart + " - Length: " +
            ab.byteLength + ")",
        );
...
MultiBufferStream.prototype.logBufferLevel = function (info) {
  ...
  for (i = 0; i < this.buffers.length; i++) {
    ...
    if (i === 0) {
      ...
      range = {};
      range.start = buffer.fileStart;
      bufferedString += (ranges[ranges.length - 1].end - 1) + "], [" +
        range.start + "-";
      range.end = buffer.fileStart + buffer.byteLength;
      ranges.push(range);
    }
  ...
  if (this.buffers.length === 0) {
    ...
    log(
      "MultiBufferStream",
      "" + this.buffers.length + " stored buffer(s) (" + used + "/" + total +
        " bytes), continuous ranges: " + bufferedString,
    );
  }
};
...
MultiBufferStream.prototype.mergeNextBuffer = function () {
  ...
  if (this.bufferIndex + 1 < this.buffers.length) {
    ...
    if (
      next_buffer.fileStart === this.buffer.fileStart + this.buffer.byteLength
    ) {
      ...
      Log.debug(
        "ISOFile",
        "Concatenating buffer for box parsing (length: " + oldLength + "->" +
...
var MPEG4DescriptorParser = function () {
  ...
  this.parseOneDescriptor = function (stream) {
    ...
    Log.debug(
      ...
      "Found " + (descTagToName[tag] || "Descriptor " + tag) + ", size " +
        size + " at position " + stream.getPosition(),
    );
  ...
  classes.ES_Descriptor.prototype.getAudioConfig = function (stream) {
    ...
    if (dsi && dsi.data) {
      ...
      if (audioObjectType === 31 && dsi.data.length >= 2) {
        audioObjectType = 32 + ((dsi.data[0] & 0x7) << 3) +
          ((dsi.data[1] & 0xE0) >> 5);
      }
...
var BoxParser = {
  ...
  createMediaSampleEntryCtor: function (mediaType, parseMethod, subBoxNames) {
    ...
    BoxParser[mediaType + "SampleEntry"].prototype = new BoxParser
      .SampleEntry();
    if (parseMethod) {
  ...
  createSampleEntryCtor: function (mediaType, type, parseMethod, subBoxNames) {
    ...
    BoxParser[type + "SampleEntry"].prototype =
      new BoxParser[mediaType + "SampleEntry"]();
    if (parseMethod) {
  ...
  createSampleGroupCtor: function (type, parseMethod) {
    ...
    BoxParser[type + "SampleGroupEntry"].prototype = new BoxParser
      .SampleGroupEntry();
    if (parseMethod) {
  ...
  createTrackGroupCtor: function (type, parseMethod) {
    ...
    BoxParser[type + "TrackGroupTypeBox"].prototype = new BoxParser
      .TrackGroupTypeBox();
    if (parseMethod) {
  ...
  createUUIDBox: function (uuid, isFullBox, isContainerBox, parseMethod) {
    ...
    BoxParser.UUIDBoxes[uuid].prototype = isFullBox
      ...
      : (isContainerBox ? new BoxParser.ContainerBox() : new BoxParser.Box());
    if (parseMethod) {
      if (isFullBox) {
...
BoxParser.parseOneBox = function (stream, headerOnly, parentSize) {
  ...
  Log.debug(
    "BoxParser",
    "Found box of type '" + type + "' and size " + size + " at position " +
      start,
  );
  hdr_size = 8;
  if (type == "uuid") {
  ...
  if (size == 1) {
    if (
      ...
      Log.warn(
        "BoxParser",
        'Not enough data in stream to parse the extended size of the "' + type +
          '" box',
      );
      return { code: BoxParser.ERR_NOT_ENOUGH_DATA };
    }
  ...
  if (size !== 0 && size < hdr_size) {
    Log.error(
      "BoxParser",
      "Box of type " + type + " has an invalid size " + size +
  ...
  if (size !== 0 && parentSize && size > parentSize) {
    Log.error(
      "BoxParser",
      "Box of type '" + type + "' has a size " + size +
        " greater than its container size " + parentSize,
    );
    return {
      code: BoxParser.ERR_NOT_ENOUGH_DATA,
      type: type,
  ...
  if (diff < 0) {
    Log.warn(
      "BoxParser",
      "Parsing of box '" + box_type +
        "' did not read the entire indicated box data size (missing " +
        (-diff) + " bytes), seeking forward",
    );
    stream.seek(box.start + box.size);
  } else if (diff > 0) {
    Log.error(
      "BoxParser",
      "Parsing of box '" + box_type + "' read " + diff +
...
BoxParser.createFullBoxCtor("auxC", function (stream) {
  ...
  var aux_subtype_length = this.size - this.hdr_size -
    (this.aux_type.length + 1);
...
BoxParser.createBoxCtor("mdcv", function (stream) {
  ...
  this.display_primaries[0] = new ColorPoint(
    stream.readUint16(),
    stream.readUint16(),
  );
  this.display_primaries[1] = new ColorPoint(
    stream.readUint16(),
    stream.readUint16(),
  );
  this.display_primaries[2] = new ColorPoint(
    stream.readUint16(),
    stream.readUint16(),
  );
...
BoxParser.createSampleGroupCtor("rash", function (stream) {
  ...
  if (
    this.description_length !==
      2 + (this.operation_point_count === 1
          ? 2
...
BoxParser.createFullBoxCtor("sgpd", function (stream) {
  ...
  for (var i = 0; i < entry_count; i++) {
    ...
    if (entry.write === BoxParser.SampleGroupEntry.prototype.write) {
      Log.info(
        "BoxParser",
        "SampleGroup for type " + this.grouping_type +
...
BoxParser.createFullBoxCtor("vvcC", function (stream) {
  ...
  var bitReader = {
    ...
    extract_bits: function (num_bits) {
      var ret = (this.held_bits >> (this.num_held_bits - num_bits)) &
        ((1 << num_bits) - 1);
...
BoxParser.mp4aSampleEntry.prototype.getCodec = function () {
  ...
  if (this.esds && this.esds.esd) {
    ...
    return baseCodec + "." + BoxParser.decimalToHex(oti) +
      (dsi ? "." + dsi : "");
  } else {
    return baseCodec;
  }
...
BoxParser.vp08SampleEntry.prototype.getCodec =
  BoxParser.vp09SampleEntry.prototype.getCodec =
    function () {
      ...
      return baseCodec + ".0" + this.vpcC.profile + "." + level + "." +
        bitDepth;
    };
...
BoxParser.av01SampleEntry.prototype.getCodec = function () {
  ...
  return baseCodec + "." + this.av1C.seq_profile + "." + level +
    (this.av1C.seq_tier_0 ? "H" : "M") + "." + bitdepth; //+"."+this.av1C.monochrome+"."+this.av1C.chroma_subsampling_x+""+this.av1C.chroma_subsampling_y+""+this.av1C.chroma_sample_position;
};
...
BoxParser.Box.prototype.writeHeader = function (stream, msg) {
  ...
  Log.debug(
    "BoxWriter",
    "Writing box " + this.type + " of size: " + this.size + " at position " +
      stream.getPosition() + (msg || ""),
  );
  if (this.size > MAX_SIZE) {
...
BoxParser.emsgBox.prototype.write = function (stream) {
  this.version = 0;
  this.flags = 0;
  this.size = 4 * 4 + this.message_data.length +
    (this.scheme_id_uri.length + 1) + (this.value.length + 1);
  this.writeHeader(stream);
...
BoxParser.hvcCBox.prototype.write = function (stream) {
  ...
  for (i = 0; i < this.nalu_arrays.length; i++) {
    ...
    stream.writeUint8(
      (this.nalu_arrays[i].completeness << 7) +
...
BoxParser.boxEqualFields = function (box_a, box_b) {
  ...
  for (prop in box_a) {
    if (BoxParser.DIFF_BOXES_PROP_NAMES.indexOf(prop) > -1) {
      ...
      typeof box_a[prop] === "undefined" || typeof box_b[prop] === "undefined"
    ) {
      continue;
    } else if (
      typeof box_a[prop] === "function" || typeof box_b[prop] === "function"
    ) {
      continue;
    } else if (
      ...
      if (
        prop === "data" || prop === "start" || prop === "size" ||
        prop === "creation_time" || prop === "modification_time"
      ) {
        continue;
        ...
        switch (box_type) {
          ...
          default:
            if (this[box_type] !== undefined) {
              Log.warn(
                "ISOFile",
                "Duplicate Box of type: " + box_type +
...
ISOFile.prototype.appendBuffer = function (ab, last) {
  ...
  if (this.stream.cleanBuffers) {
    Log.info(
      "ISOFile",
      "Done processing buffer (fileStart: " + ab.fileStart +
...
ISOFile.prototype.getInfo = function () {
  ...
  if (this.moov) {
    ...
    for (i = 0; i < this.moov.traks.length; i++) {
      ...
      track.kind = trak.udta && trak.udta.kinds.length
        ? trak.udta.kinds[0]
        : { schemeURI: "", value: "" };
      ...
      track.bitrate = (track.size * 8 * track.timescale) /
...
ISOFile.prototype.processSamples = function (last) {
  ...
  if (this.isFragmentationInitialized && this.onSegment !== null) {
    for (i = 0; i < this.fragmentedTracks.length; i++) {
      ...
      trak = fragTrak.trak;
      while (
        trak.nextSample < trak.samples.length && this.sampleProcessingStarted
      ) {
        /* The sample information is there (either because the file is not fragmented and this is not the last sample,
				or because the file is fragmented and the moof for that sample has been received */
        Log.debug(
          "ISOFile",
          "Creating media fragment on track #" + fragTrak.id + " for sample " +
            trak.nextSample,
        );
        ...
        if (
          ...
          Log.info(
            "ISOFile",
            "Sending fragmented data on track #" + fragTrak.id +
              " for samples [" +
  ...
  if (this.onSamples !== null) {
    ...
    for (i = 0; i < this.extractedTracks.length; i++) {
      ...
      while (
        ...
        Log.debug(
          "ISOFile",
          "Exporting on track #" + extractTrak.id + " sample #" +
            trak.nextSample,
        );
        ...
        if (
          ...
          Log.debug(
            "ISOFile",
            "Sending samples on track #" + extractTrak.id + " for sample " +
              trak.nextSample,
          );
          if (this.onSamples) {
            this.onSamples(
...
ISOFile.prototype.releaseUsedSamples = function (id, sampleNum) {
  ...
  Log.info(
    "ISOFile",
    "Track #" + id + " released samples up to " + sampleNum +
      " (released size: " + size + ", remaining: " + this.samplesDataSize + ")",
  );
...
ISOFile.prototype.seekTrack = function (time, useRap, trak) {
  ...
  if (trak.samples.length === 0) {
    Log.info(
      ...
      "No sample in track, cannot seek! Using time " +
        Log.getDurationString(0, 1) + " and offset: " + 0,
    );
  ...
  seek_offset = trak.samples[seek_sample_num].offset +
    trak.samples[seek_sample_num].alreadyRead;
  Log.info(
    "ISOFile",
    "Seeking to " + (useRap ? "RAP" : "") + " sample #" + trak.nextSample +
      " on track " + trak.tkhd.track_id + ", time " +
      Log.getDurationString(time, timescale) + " and offset: " + seek_offset,
  );
...
ISOFile.prototype.seek = function (time, useRap) {
  ...
  if (!this.moov) {
    ...
    Log.info(
      "ISOFile",
      "Seeking at time " + Log.getDurationString(seek_info.time, 1) +
        " needs a buffer with a fileStart position of " + seek_info.offset,
    );
...
ISOFile.prototype.init = function (_options) {
  ...
  const secondsBetween1904and1970 = 2082844800;
  const nowInSeconds = Math.floor(Date.now() / 1000) -
    secondsBetween1904and1970;

  var ftyp = this.add("ftyp").set(
    "major_brand",
    (options.brands && options.brands[0]) || "iso4",
  )
    .set("minor_version", 0)
  ...
  moov.add("mvhd").set("timescale", options.timescale || 600)
    ...
    .set("creation_time", nowInSeconds)
    .set("modification_time", nowInSeconds)
...
ISOFile.prototype.addTrack = function (_options) {
  ...
  minf.add("dinf").add("dref").addEntry(
    (new BoxParser["url Box"]()).set("flags", 0x1),
  );
...
ISOFile.initSampleGroups = function (
  ...
  for (k = 0; k < sbgps.length; k++) {
    sample_group_key = sbgps[k].grouping_type + "/" +
    ...
    if (sample_number >= sample_groups_info[k].last_sample_in_run) {
      ...
      if (
        ...
        sample_groups_info[k].last_sample_in_run +=
          sample_groups_info[k].sbgp.entries[sample_groups_info[k].entry_index]
            .sample_count;
      }
    }
    if (
      ...
      if (sample.sample_groups[k].group_description_index > 0) {
        ...
        if (description && description.version >= 2) {
          if (description.default_group_description_index > 0) {
            sample.sample_groups[k].description = description
              .entries[description.default_group_description_index - 1];
          }
        }
      }
    }
...
ISOFile.prototype.buildTrakSampleLists = function (trak) {
  ...
  for (j = 0; j < stsz.sample_sizes.length; j++) {
    ...
    if (j === 0) {
      chunk_index =
        1; /* the first sample is in the first chunk (chunk indexes are 1-based) */
      chunk_run_index =
        0; /* the first chunk is the first entry in the first_chunk table */
    ...
    sample.offset = stco.chunk_offsets[sample.chunk_index - 1] +
        ...
        for (j = 0; j < traf.truns.length; j++) {
          ...
          for (k = 0; k < trun.sample_count; k++) {
            ...
            if (trak.first_traf_merged || k > 0) {
              sample.dts = trak.samples[trak.samples.length - 2].dts +
            ...
            if (trun.flags & BoxParser.TRUN_FLAGS_FLAGS) {
              ...
              k === 0 && (trun.flags & BoxParser.TRUN_FLAGS_FIRST_FLAG)
            ) {
            ...
            var bdop = (traf.tfhd.flags & BoxParser.TFHD_FLAG_BASE_DATA_OFFSET)
              ? true
              : false;
            var dbim =
              (traf.tfhd.flags & BoxParser.TFHD_FLAG_DEFAULT_BASE_IS_MOOF)
                ? true
                : false;
            var dop = (trun.flags & BoxParser.TRUN_FLAGS_DATA_OFFSET)
              ? true
              : false;
            var bdo = 0;
            if (!bdop) {
            ...
            if (
              traf.sbgps.length > 0 || traf.sgpds.length > 0 ||
...
ISOFile.prototype.getSample = function (trak, sampleNum) {
  ...
  if (!sample.data) {
    ...
    Log.debug(
      "ISOFile",
      "Allocating sample #" + sampleNum + " on track #" + trak.tkhd.track_id +
        " of size " + sample.size + " (total: " + this.samplesDataSize + ")",
    );
  ...
  while (true) {
    ...
    if (index > -1) {
      ...
      var lengthAfterStart = buffer.byteLength -
      ...
      if (sample.size - sample.alreadyRead <= lengthAfterStart) {
        ...
        Log.debug(
          "ISOFile",
          "Getting sample #" + sampleNum + " data (alreadyRead: " +
            sample.alreadyRead + " offset: " +
            (sample.offset + sample.alreadyRead - buffer.fileStart) +
            " read size: " + (sample.size - sample.alreadyRead) +
        ...
        Log.debug(
          "ISOFile",
          "Getting sample #" + sampleNum + " partial data (alreadyRead: " +
            sample.alreadyRead + " offset: " +
            (sample.offset + sample.alreadyRead - buffer.fileStart) +
            " read size: " + lengthAfterStart + " full size: " + sample.size +
            ")",
        );
  ...
  if (meta.iloc) {
    for (i = 0; i < meta.iloc.items.length; i++) {
      ...
      for (j = 0; j < itemloc.extents.length; j++) {
        item.extents[j] = {};
        item.extents[j].offset = itemloc.extents[j].extent_offset +
          itemloc.base_offset;
...
ISOFile.prototype.getItem = function (item_id) {
  ...
  if (!item.data && item.size) {
    ...
    Log.debug(
      "ISOFile",
      "Allocating item #" + item_id + " of size " + item.size + " (total: " +
        this.itemsDataSize + ")",
    );
    ...
    if (extent.alreadyRead === extent.length) {
      ...
      if (index > -1) {
        ...
        if (extent.length - extent.alreadyRead <= lengthAfterStart) {
          ...
          Log.debug(
            "ISOFile",
            "Getting item #" + item_id + " extent #" + i +
              " data (alreadyRead: " + extent.alreadyRead +
              " offset: " +
              (extent.offset + extent.alreadyRead - buffer.fileStart) +
              " read size: " + (extent.length - extent.alreadyRead) +
              " full extent size: " + extent.length + " full item size: " +
              item.size + ")",
          );
          ...
          Log.debug(
            "ISOFile",
            "Getting item #" + item_id + " extent #" + i +
              " partial data (alreadyRead: " + extent.alreadyRead +
              " offset: " +
              (extent.offset + extent.alreadyRead - buffer.fileStart) +
              " read size: " + lengthAfterStart +
              " full extent size: " + extent.length + " full item size: " +
              item.size + ")",
          );
...
MP4Box.createFile = function (_keepMdatData, _stream, bfOptions) {
