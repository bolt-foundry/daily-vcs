...
export function streamFileToOpfs(file: File, fileName: string): rxjs.Observable<OpfsProgressEvent | OpfsCompletionEvent | OpfsErrorEvent> {
  const subject = new Subject<OpfsProgressEvent | OpfsCompletionEvent | OpfsErrorEvent>();
  const fileMetadata: FileMetadata = { name: fileName, size: file.size };

  (async () => {
    try {
      const opfsDirectory = await navigator.storage.getDirectory();
      const opfsFile = await opfsDirectory.getFileHandle(fileName, { create: true });
      ...
      while (true) {
        ...
        subject.next({ type: "progress", file: fileMetadata, bytesWritten: value.length, totalBytesWritten: totalWritten });
      }

      await writableStream.close();
      logger.log("File successfully written to Opfs");

      subject.next({ type: "completion", file: fileMetadata, message: "File successfully written to Opfs" });
      subject.complete();
    } catch (error) {
      subject.error({ type: "error", file: fileMetadata, error: new ErrorOpfsGenericError(error.message) });
    }
  })();
