...
// } from "packages/__generated__/ClipList_project.graphql.ts";
// import { useClipEditData_clip$key } from "packages/__generated__/useClipEditData_clip.graphql.ts";
import { Clip } from "packages/client/components/Clip.tsx";
...
import { TranscriptView } from "packages/client/components/TranscriptView.tsx";
import { ClipEdit } from "packages/client/components/ClipEdit.tsx";
...
export function ClipList({ project$key, gotoClip, videoSrc }: Props) {
  ...
  const loadNext = (num: number) => console.log("loadNext", num);
  const hasNext = data.clips.pageInfo.hasNextPage;
  const isLoadingNext = false;
  const refetch = (id: number) => console.log("refetch", id);
